<template>
    ...
    <div v-else-if="is518Old" >
      <q-banner inline-actions rounded class="bg-warning" >
        WARNING: The Alpaca Driver is not receiving position updates from Polaris. Check Setup, Wi-Fi or CPU load. 
        <template v-slot:action><q-btn v-if="isShowReconnect" flat label="Reconnect" to="/connect" /></template>
      </q-banner>
    </div>
    <div v-else-if="is517Old" >
      <q-banner inline-actions rounded class="bg-warning" >
        WARNING: The Alpaca Driver is not receiving orientation updates from Polaris. Check Setup, Wi-Fi or CPU load. 
        <template v-slot:action><q-btn v-if="isShowReconnect" flat label="Reconnect" to="/connect" /></template>
      </q-banner>
    </div>
