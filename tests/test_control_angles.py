...
def test_north_pole_reference_cases():
    ...
    assert pytest.approx(polar_rotation_angle(lat, az, deg2rad(70)), abs=1e-6) == 0.0
    assert pytest.approx(polar_rotation_angle(lat, az, deg2rad(30)), abs=1e-6) == 180.0
...
def test_south_pole_reference_cases():
    ...
    assert pytest.approx(polar_rotation_angle(lat, az, deg2rad(70)), abs=1e-6) == 0.0
    assert pytest.approx(polar_rotation_angle(lat, az, deg2rad(10)), abs=1e-6) == 180.0
    assert pytest.approx(polar_rotation_angle(lat, az, pole_alt), abs=1e-6) == 0.0

def test_rotation_angle_east_west_of_celestrial_pole():
    lat = deg2rad(33.86)
    pole_alt = abs(lat)
    assert pytest.approx(polar_rotation_angle(lat, deg2rad(90), pole_alt), abs=1e-6) == 90.0
    assert pytest.approx(polar_rotation_angle(lat, deg2rad(270), pole_alt), abs=1e-6) == -90.0
