...
def test_misc_roundtrip_theta_q1(theta1, theta2, theta3, q1):
    assert str(theta_to_q(theta1, theta2, theta3)) == str(q1)
    assert (theta1, theta2, theta3) == tuple(round(x) for x in q_to_theta(q1, lastPos=LastPosition(180,30,0)))
...
def test_5t1_30t2_roundtrip_theta_q1(theta1, theta2, theta3, q1):
    assert str(theta_to_q(theta1, theta2, theta3)) == str(q1)
    assert (theta1, theta2, theta3) == tuple(round(x) for x in q_to_theta(q1, lastPos=LastPosition(+5,+30,0)))
...
def test_355t1_30t2_roundtrip_theta_q1(theta1, theta2, theta3, q1):
    assert str(theta_to_q(theta1, theta2, theta3)) == str(q1)
    assert (theta1, theta2, theta3) == tuple(round(x) for x in q_to_theta(q1, lastPos=LastPosition(355,+30,0)))
...
def test_355t1_minus7t2_roundtrip_theta_q1(theta1, theta2, theta3, q1):
    assert str(theta_to_q(theta1, theta2, theta3)) == str(q1)
    assert (theta1, theta2, theta3) == tuple(round(x) for x in q_to_theta(q1, lastPos=LastPosition(355,-7,0)))
...
def test_5t1_0t2_roundtrip_theta_q1(theta1, theta2, theta3, q1, t1,t2,t3):
    assert str(theta_to_q(theta1, theta2, theta3)) == str(q1)
    assert (t1, t2, t3) == tuple(round(x) for x in q_to_theta(q1))
...
def test_90t1_minus5t2_roundtrip_theta_q1(theta1, theta2, theta3, q1):
    assert str(theta_to_q(theta1, theta2, theta3)) == str(q1)
    assert (theta1, theta2, theta3) == tuple(round(x) for x in q_to_theta(q1, lastPos=LastPosition(90,-5,80)))
...
def test_180t1_70t2_roundtrip_theta_q1(theta1, theta2, theta3, q1):
    assert str(theta_to_q(theta1, theta2, theta3)) == str(q1)
    assert (theta1, theta2, theta3) == tuple(round(x) for x in q_to_theta(q1, lastPos=LastPosition(180,70,0)))


def test_azaltroll_to_q():
    assert str(azaltroll_to_q(+97.0875,+44.7835,-4.9968)) == str(Quaternion(-0.382, +0.017, +0.923, +0.040))  # 90, 45, -5
    assert str(azaltroll_to_q(+82.9540,+44.7845,+4.9851)) == str(Quaternion(+0.382, +0.017, -0.923, +0.040))  # 90, 45, +5
    #assert str(azaltroll_to_q(+354.9565,+44.7846,+4.9850)) == str(Quaternion(+0.247, +0.653, -0.652, +0.295)) # 2, 45, -5 ** wrong sign
    assert str(azaltroll_to_q(+9.0678,+44.7843,-4.9894)) == str(Quaternion(-0.303, -0.629, +0.676, -0.237))   # 2, 45, +5
    #assert str(azaltroll_to_q(+350.9593,+44.7846,+4.9830)) == str(Quaternion(+0.237, +0.675, -0.629, +0.303))  # 358, 45, -5 **wrong sign
...
def test_jacobian_matches_finite_difference(theta1, theta2, theta3):
    ...
    q0 = theta_to_q(theta1, theta2, theta3)
    theta_eps = theta + theta_dot_deg * dt
    q1 = theta_to_q(*theta_eps)
